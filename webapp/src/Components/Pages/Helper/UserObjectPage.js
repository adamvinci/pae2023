...
<div id="carouselExampleControls" class="carousel carousel-dark slide"  data-bs-ride="carousel" data-bs-interval="10000000">
  <div class="carousel-inner">
    <div class="carousel-item active">
      <img id = "imgCarousel" class="d-block " src=/api/objet/getPicture/${objets[0].idObjet}  data-x = ${0} alt=${objets[0].description} >
  ...
  objets2.slice(1).forEach((o) => {
    html += `      
      ...
      <img id = "imgCarousel" class="d-block  " src=/api/objet/getPicture/${o.idObjet} data-x = ${objets.indexOf(
        o)} alt=${o.description} >
               <div class="carousel-caption d-none d-md-block">
          ...
          <div class = "detail">
           <table>
              ...
              <tbody class="tableData">
                <tr>
                  <td class="rechercheObjetsTd">${objData.localisation
          ? `${objData.localisation}` : '/'}</td> 
                  <td class="rechercheObjetsTd">${objData.typeObjet.libelle
          ? `${objData.typeObjet.libelle}` : '/'}</td>
                  <td class="rechercheObjetsTd">${objData.date_depot
          ? ` ${objData.date_depot[2]}/${objData.date_depot[1]}/${objData.date_depot[0]}`
          : '/'}</td>
                  <td class="rechercheObjetsTd">${objData.date_vente
          ? ` Le ${objData.date_vente[2]}/${objData.date_vente[1]}/${objData.date_vente[0]} `
          : '/'}</td>
                  <td class="rechercheObjetsTd">${objData.prix
          ? `${objData.prix}  € ` : '/'} </td>
                  <td class="rechercheObjetsTd">${objData.etat
          ? `${objData.etat}` : '/'}</td>
                </tr>
              </tbody>    
            </table>
